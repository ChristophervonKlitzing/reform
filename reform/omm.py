...
class OMMTReplicas(MultiTReplicas):
    ...
    def _create_contexts(self):
        ...
        for i in range(self._N):
            integrator = omm.LangevinIntegrator(
                self._temps[i] * unit.kelvin, friction, time_step
            )
            ...
            context = omm.Context(
                self._system, integrator, self._platform, self._platform_prop
            )
    ...
    def minimize_energy(
        ...
        omm.LocalEnergyMinimizer.minimize(
            self._contexts[index], tolerance, max_iterations
        )
    ...
    def _vel_scaling_factor(self, ori_index, target_index) -> float:
        ...
        if self._integrator_params["integrator"] != "Langevin":
            raise NotImplementedError(
                f"Scaling with integrator {self._integrator_params['integrator']} is not yet "
                f"supported."
            )
    ...
    def exchange_pair(self, pair: Tuple[int, int]):
        ...
        self._contexts[b].setVelocities(
            state_a.getVelocities() * self._vel_scaling_factor(a, b)
        )
        self._contexts[a].setPositions(state_b.getPositions())
        self._contexts[a].setVelocities(
            state_b.getVelocities() * self._vel_scaling_factor(b, a)
        )
    ...
    def get_state(
        ...
        """Return a openmm.State object corresponding to replica #index"""
        self._check_index(index)
        state = self._contexts[index].getState(
    ...
    def get_states(
        ...
        """Return a list of openmm.State object corresponding to all replicas"""
        return [
            self._contexts[i].getState(
                getPositions=getPositions,
                getVelocities=getVelocities,
                getForces=getForces,
                getEnergy=getEnergy,
