...
class OMMTReplicas_replicated(OMMTReplicas):
    def __init__(
        ...
        super(OMMTReplicas_replicated, self).__init__(
    ...
    def _create_contexts(self):
        ...
        if self._integrator_params["integrator"] != "Langevin":
            raise NotImplementedError(
                f"Integrator {self._integrator_params['integrator']} is not implemented."
            )
        ...
        self._single_integrator = omm.LangevinIntegrator(
            self._temps[0] * unit.kelvin, friction, time_step
        )
        if "constraint_tolerance" in self._integrator_params.keys():
            self._single_integrator.setConstraintTolerance(
                self._integrator_params["constraint_tolerance"]
            )
        else:
            self._single_integrator.setConstraintTolerance(0.00001)
        self._single_context = omm.Context(
            self._system, self._single_integrator, self._platform, self._platform_prop
        )
        self._single_context_positions_set = False

        # initiate the replicated context (for simulation)
        self._replicated_system = ReplicatedSystem.replicate_system(
            self._system, self._N
        )
        for f in self._additional_forces:
            self._replicated_system.addForce(f)
        temperatures_per_dof = np.concatenate(
            [
                np.ones((self._system.getNumParticles(), 3)) * temp
                for temp in self._temps
            ]
        )
        integrator = get_custom_langevin_integrator(
            temperatures_per_dof,
            self._integrator_params["friction_in_inv_ps"],
        ...
        context = omm.Context(
            self._replicated_system, integrator, self._platform, self._platform_prop
        )
    ...
    def get_states(
        self, getPositions=False, getVelocities=False, getForces=False, getEnergy=False
    ) -> List[omm.State]:
        """Not efficient for the replicated system. Deprecated."""
        return NotImplemented
    ...
    def get_rep_state(
        ...
        return self._contexts[0].getState(
    ...
    def get_positions_all(self) -> unit.Quantity:
        return (
            self.get_rep_state(getPositions=True)
    ...
    def get_velocities_all(self) -> unit.Quantity:
        return (
            self.get_rep_state(getVelocities=True)
    ...
    def set_velocities(self, index: int, velocities=None):
        ...
        if velocities is not None:
            ...
            velocities = self._single_context.getState(
                getVelocities=True
            ).getVelocities(asNumpy=True)
            self.set_velocities(index, velocities)

    def minimize_energy_all(
        self, tolerance=10 * unit.kilojoule / unit.mole, max_iterations: int = 0
    ):
        ...
        assert (
            max_iterations >= 0
        ), "Invalid number for iterations, should be non-negative."
    ...
    def minimize_energy(
        ...
        omm.LocalEnergyMinimizer.minimize(
            self._single_context, tolerance, max_iterations
        )
        minimized_posi = self._single_context.getState(getPositions=True).getPositions(
            asNumpy=True
        )
    ...
    def load_states(self, filepath: str = "./omm_chkpt.npz", check_temps=True):
        ...
        if check_temps:
            assert np.allclose(chkpt["set_temps"], self._temps), (
                "Preset temperatures in the checkpoint are different"
                " from current OMMTReplicas object."
            )
        assert self._N == len(chkpt["positions"]) and self._N == len(
            chkpt["velocities"]
        ), "Checkpoint file is invalid: number of replicas is not consistent inside the file."
        assert len(chkpt["set_temps"]) == len(self._temps), (
            "Checkpoint file is incompatible: number of replicas is "
            "not consistent with the current OMMTReplicas object."
        )
        assert chkpt["positions"].shape[1] == self._system.getNumParticles(), (
            "Number of particles in the checkpoint "
