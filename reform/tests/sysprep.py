...
def prepare_capped_alanine_replicas(
    temps_intended, replicated=True
) -> omm.OMMTReplicas:
    ...
    if replicated:
        replicas = omm_replicated.OMMTReplicas_replicated(
            system, temps_intended, integrator_params=integrator_params
        )
        replicas.set_positions_all([pdb.getPositions()] * len(temps_intended))
    else:
        replicas = omm.OMMTReplicas(
            system, temps_intended, integrator_params=integrator_params
        )
    ...
    # set velocities
    for i in range(replicas.num_replicas):
        replicas.set_velocities(
            i
        )  # will be redirected to a OpenMM internal function to set system velocities to temp
