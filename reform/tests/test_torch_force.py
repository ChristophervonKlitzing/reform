...
def _prepare_capped_alanine_simu_torch(
    temps_intended, torch_force_func, interface="single_threaded"
) -> simu_utils.MultiTSimulation:
    ...
    if interface == "single_threaded":
        ...
        simu = simu_utils.MultiTSimulation(
            system,
            temps_intended,
            replicated_system_additional_forces=[f],
            interface=interface,
            platform="CUDA",
            integrator_params=integrator_params,
            verbose=False,
...
def test_full_simulation_simu_w_torch(interface):
    ...
    recorder_hook = simu_utils.NpyRecorderHook(
        "/tmp/00100.npy", int(total_steps / RECORDING_INTERVAL), 100
    )
    ...
    assert recorded.shape[0] == len(
        temps_intended
    ), "Wrong number of replicas in recorded trajectories."
